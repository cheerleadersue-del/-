...
```
assets/hero.mp4          ← PC · 태블릿용 (가로)
assets/hero-mobile.mp4   ← 휴대폰용 (세로). 있으면 좁은 화면에서 이것을 씁니다
assets/hero.webm         ← mp4 가 없을 때만
```

**휴대폰용을 따로 올리시는 것을 권합니다.** 가로 영상 하나만 두면
세로 화면에서는 가운데만 크게 잘립니다. 휴대폰용이 없으면 가로
영상을 그대로 쓰므로, 굳이 안 만드셔도 화면이 깨지지는 않습니다.

만드실 때 권하는 값

| | PC용 `hero.mp4` | 휴대폰용 `hero-mobile.mp4` |
|---|---|---|
| 비율 | 가로 16:9 | 세로 9:16 |
| 크기 | 1920 × 1080 이상 | 1080 × 1920 이상 |
| 길이 | 10 ~ 15초 | 10 ~ 15초 |
| 용량 | 3MB 이하 | 3MB 이하 |
| 소리 | 없음 | 없음 |

움직임은 아주 느려야 합니다. 빠르면 위에 얹힌 글자가 읽히지 않습니다.
길이가 짧으면 반복되는 것이 눈에 띕니다.
...
| 대표전화 · 공증 상담 | 02-567-4377 |
| 상담 | 010-8227-8485 (휴대전화 · 카카오톡) — 공증 외 전 분야 |
| 팩스 | 02-583-5222 |
| 공증팩스 | 02-563-8181 |
