...
경력은 모두 들어갔습니다. **사진 파일만 올리시면 됩니다.**

`assets/` 에 **`lawyer-06`** 이라는 이름으로 올리세요.
확장자는 `.webp` · `.jpg` · `.jpeg` · `.png` 아무거나 됩니다.
코드가 순서대로 찾아보고, 없으면 자리표시자를 대신 보여줍니다.

```
assets/lawyer-06.webp   또는  .jpg / .jpeg / .png
```

세로로 긴 인물사진이 좋습니다. 카드가 3 : 3.7 비율로 잘라 씁니다.

이 확장자 자동 탐색은 **모든 변호사 사진에 적용됩니다.**
다른 분 사진을 바꾸실 때도 이름만 맞추면 됩니다.
...
| 2 | 양진희 변호사 | `lawyer-06.webp` **(사진 대기)** | 변시 11회 · 고려대 · 前 법무법인 송우 |
...
- 형사의 `topics` 는 세부 분야 목록입니다. `href` 를 채우면 링크가 되고,
  **비워두면 링크 없이 글자로만** 나옵니다. 아직 상세 페이지가 없어 전부 비어 있습니다.
