...
- 히어로 인물
  - 980px 이하 — 텍스트 오른쪽에 배경으로 깔림
  - **620px 이하 — 배경에서 꺼내 문구 아래에 세로로 세움** (380px)
- 히어로 칩과 업무분야 탭은 **가로 스와이프**
- 변호사 카드는 620px 이하에서 **왼쪽 사진(3:4) · 오른쪽 정보** 2단으로 분리.
  사진 위에 그라데이션·워터마크를 덮지 않으며 경력은 항상 노출
- 약속 카드는 1열
...
- 인물 사진은 **세로 비율**입니다. 좁은 화면에서 `aspect-ratio` 를 가로로
  눕히면 눈높이 한 줄만 남습니다. 좁아질수록 칸을 나누되 비율은 지킵니다.
- `assets/lawyer-01.webp` 는 배경을 지운 **컷아웃**입니다.
  `object-fit` 은 `cover` 가 아니라 `contain` 이어야 인물이 잘리지 않습니다.
- `.hero-scroll` 은 히어로가 화면 전체를 채울 때만 씁니다. 980px 이하에서는
  `min-height` 가 풀려 히어로가 짧아지므로 칩 위에 겹칩니다. 숨깁니다.
