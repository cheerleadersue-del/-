...
| 5 | 변호인단 | 6인 3열 그리드, 사진 위에 아무것도 안 덮음 |
| 6 | 원칙 | 어두운 구간 |
...
지금은 `assets/lawyer-06.webp` 가 없어서 자리표시자(`lawyer-placeholder.svg`)가
대신 보입니다. 파일을 올리시면 **코드를 고치지 않아도 자동으로 바뀝니다.**

1. 사진을 컴퓨터에 저장
2. 저장소 `assets/` 폴더 → **Add file** → **Upload files**
3. 파일 이름을 **`lawyer-06.webp`** 로 맞춰 올리기
   (png·jpg 로 올리시려면 `main.js` 의 `image` 값도 그 확장자로 바꿔야 합니다)
4. Commit
