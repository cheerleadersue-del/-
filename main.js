...
/* ---------------------------------------------------------------------
   ...
   받는 곳은 2026-09-24 에 대표님이 정하셨습니다. 아래 주소 그대로
   둡니다. 법인 메일(psg8656@naver.com)이 아니라 이쪽입니다.

   2026-09-24. 확인 메일을 누르셨고, formsubmit 이 준 암호 주소로
   바꿔 두었습니다. 아래 줄에 이메일이 드러나 있지 않습니다.
   전에는 지메일 주소가 그대로 적혀 있어 스팸 수집 프로그램이
   긁어갈 수 있었습니다. 이제 받는 곳은 formsubmit 쪽에만 있습니다.

   ⚠️ 그러므로 이 줄을 다시 「formsubmit.co/(이메일)」 꼴로 되돌리지
      마십시오. 되돌리는 순간 이메일이 다시 드러납니다.

   ⚠️ 받는 곳을 바꾸시려면 formsubmit 화면에서 바꾸시거나, 새 주소로
      다시 한 번 만들어 이 줄을 갈아끼우십시오. 새로 만드시면
      확인 메일을 한 번 더 눌러주셔야 합니다.
--------------------------------------------------------------------- */
const FORM_ENDPOINT = "https://formsubmit.co/0e6209c0cf4f12db661fc73a5966e290";
  ...
  */
  {
    ...
    page: "civil.html",
    /*
      이 줄은 칸 이름 바로 밑에 붙어 "누가 맡는가" 를 먼저 보여준다.
      차례가 곧 무게라서, 민사 중심으로 바꾸면서 건설 · 부동산을
      맡는 양진희 변호사를 맨 앞에 두었다.
    */
    /*
      이름 한 사람씩 nb 로 묶어 이름과 괄호가 갈라지지 않게 하고,
      가운뎃점 앞의 빈칸은 &nbsp; 로 앞말에 붙인다.

      그냥 두면 좁은 화면에서 "정주현 변호사 / (부동산)" 처럼 괄호만
      다음 줄로 넘어가거나 "· 김제도 변호사" 처럼 점이 줄머리에 선다.
      이렇게 묶어 두면 줄은 언제나 사람과 사람 사이에서만 넘어간다.
    */
    credit: "<span class=\"nb\">양진희 변호사(건설&nbsp;· 부동산)</span>&nbsp;· " +
            "<span class=\"nb\">정주현 변호사(부동산)</span>&nbsp;· " +
            "<span class=\"nb\">김제도 변호사(의료사고)</span>",
...
/*
  ⚠️ "전문" 이라는 표현은 대한변협 전문분야 등록을 마친 경우에만 쓸 수 있습니다.
     지금 명단에는 등록이 확인된 분이 없어 아무 곳에도 쓰지 않았습니다.
     등록하신 분이 계시면 그때 field 문구에 넣으시면 됩니다.
*/
const attorneys = [
  {
    name: "정호길", role: "대표변호사",
  ...
  },
  {
    ...
    career: [
      ...
      "자문 — 건설 시행사 · 부동산 신탁 다수",
      /*
        사건 표시는 '결과' 가 아니라 '무엇을 다뤘는가' 로 적는다.
        당사자 개인의 이름은 넣지 않는다. 사건의 크기는 직위만으로 전해지고,
        실명을 적으면 그분의 개인정보를 우리 광고에 쓰는 셈이 된다.
      */
      "수행 — KBS 사장 직무집행정지 가처분 사건 대리",
      "수행 — 서울시 조례안 무효확인 소송"
    ]
  },
  {
...
/* ---------------------------------------------------------------------
   ...
   2026-09-24. 영상 쪽을 6.2초로 미뤄 사진을 더 오래 보여드렸다가,
   같은 날 원래대로 4.2초로 되돌렸다. 지금은 글자와 영상이 같은 때에
   움직인다. 숫자를 둘로 나눠 둔 것은 그대로다 — 다시 사진을 길게
   하고 싶으시면 아래 HERO_VIDEO 만 늘리면 된다.

   1·2번의 시간은 style.css 의 hero-logo / heroBgIn 에 있다.
--------------------------------------------------------------------- */
const HERO_REVEAL = 4200;
const HERO_VIDEO  = 4200;
